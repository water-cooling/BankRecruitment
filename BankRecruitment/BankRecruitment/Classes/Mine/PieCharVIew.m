...
- (instancetype)initWithFrame:(CGRect)frame withYoffset:(float)Yoffset
{
...
static inline void drawArc(CGContextRef ctx, CGPoint point, float angle_start, float angle_end, UIColor* color, float radius) {
    CGContextMoveToPoint(ctx, point.x, point.y-Yoffsettemp);
    CGContextSetFillColor(ctx, CGColorGetComponents( [color CGColor]));
    CGContextAddArc(ctx, point.x, point.y-Yoffsettemp, radius,  angle_start, angle_end, 0);
...
- (void)drawRect:(CGRect)rect
{
    if (_dataArray) {
        ...
        for (PieCharModel *model in _dataArray) {
            ...
            [model.color set];
            // 起始
            CGPoint point1 = [self getPostionX:self.center.x withPostionY:self.center.y-_Yoffset withRadius:_radius - 20 withCirAngle:(radius_start + radius_end / 2)];
            ...
            // 延伸
            CGPoint point2 = [self getPostionX:self.center.x withPostionY:self.center.y-_Yoffset withRadius:_radius + 10 withCirAngle:(radius_start + radius_end / 2)];
            // 折线
            CGFloat line_x;
            CGFloat label_x;
        ...
        UILabel *centrelabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, _radius, _radius)];
        centrelabel.text = self.centreString;
        centrelabel.font = _textFont;
        centrelabel.adjustsFontSizeToFitWidth = YES;
        centrelabel.textColor = kColorDarkText;
        centrelabel.backgroundColor = [UIColor whiteColor];
        centrelabel.textAlignment = NSTextAlignmentCenter;
        [self addSubview:centrelabel];
        centrelabel.layer.cornerRadius = _radius*.5;
        centrelabel.layer.masksToBounds = YES;
        centrelabel.center = CGPointMake(self.center.x, self.center.y-_Yoffset);
    }
}
