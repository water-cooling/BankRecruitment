...
- (LoadingHUDView*)createActivityViewAtCenter:(UIActivityIndicatorViewStyle)style title:(NSString*)indiTitle
{
	...
	LoadingHUDView *HUD = [[LoadingHUDView alloc] initWithTitle:indiTitle];
...
{
	return [self createActivityViewAtCenter:style title:@"正在加载..."];
}
...
- (void)showActivityViewAtCenter
{
    ...
    if (activityView == nil){
        ...
        activityView = [self createActivityViewAtCenter:UIActivityIndicatorViewStyleWhiteLarge title:@"正在加载..."];
...
- (void)showAndHideHUDIndicatorViewAtCenter:(NSString*)indiTitle
{
    if ([[NSThread currentThread] isMainThread]) {
        [self showHUDIndicatorViewAtCenter:indiTitle];
        [self performSelector:@selector(hideHUDIndicatorViewAtCenter) withObject:nil afterDelay:30*1.5];
    }else{
...
- (void)showHUDIndicator {
    [self showHUDIndicatorViewAtCenter:nil];
}
