...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[LLMBRoundProgressView class]];
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		if (!isRoundIndicator) {
			...
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[LLMBRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(LLMBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
		...
		[indicator removeFromSuperview];
		self.indicator = nil;
    } else if (mode == MBProgressHUDModeImage && customView != indicator) {
        [indicator removeFromSuperview];
        self.indicator = customView;
        [self addSubview:indicator];
    } else if(mode == MBProgressHUD25X){
        [indicator removeFromSuperview];
        self.indicator = MB_AUTORELEASE([[UIImageView alloc] initWithImage:[UIImage imageNamed:@"progressHUDLoading"]]);
...
- (void)layoutSubviews {
    ...
    if(mode == MBProgressHUD25X){
        ...
        //modify by zhangbeibei:20150514 3.0版本去掉文字，转圈居中显示
        indicatorF.origin.x = roundf((bounds.size.width - 22) / 2) + xPos;
...
@implementation LLMBRoundProgressView {
	float _progress;
	BOOL _annular;
}
