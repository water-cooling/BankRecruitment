...
- (void)makeUIElements {
    ...
    for (int i = 0; i < 42; i++) {
        ...
        componentView.selectedDotColor = self.selectedIndicatorColor;
...
- (void)configureWeekdayHeaderView {
    ...
    [self.weekHeaderView.arrangedSubviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (canUseLocalizedStrings) {
            ...
            weekdayLabel.text = [DAYUtils stringOfWeekdayInChinese:idx + 1];
        }
    }];
}
...
- (void)configureComponentView:(DAYComponentView *)view withDay:(NSUInteger)day month:(NSUInteger)month year:(NSUInteger)year {
    ...
    //xiajq
    [self->_allEventDateList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSDate *eventDate = obj;
        NSDateComponents *todayComps = [DAYUtils dateComponentsFromDate:eventDate];
        if (todayComps.year == year && todayComps.month == month && todayComps.day == day) {
            view.dotLayer.hidden = NO;
            *stop = YES;
            return;
        }
    }];
