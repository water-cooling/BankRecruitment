...
+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType {
    ...
    //[SVProgressHUD dismissWithError:@"网络请求超时" afterDelay:15.0f];
    [[SVProgressHUD sharedView] dismissWaitViewAfterDelay:60.0f];
}
...
+ (void)showAndDismissErrorWithStatus:(NSString *)string duration:(NSTimeInterval)duration {
    [SVProgressHUD dismissWithError:string afterDelay:duration];
}
...
- (void)dismissWaitViewAfterDelay:(NSTimeInterval)seconds {
    dispatch_async(dispatch_get_main_queue(), ^{
        
        self.fadeOutTimer = [NSTimer scheduledTimerWithTimeInterval:seconds target:self selector:@selector(dismissAndShowErrorView) userInfo:nil repeats:NO];
    });
}

- (void)dismissAndShowErrorView
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [self dismissWithStatus:@"网络请求超时！" error:YES afterDelay:1.0f];
    });
}
