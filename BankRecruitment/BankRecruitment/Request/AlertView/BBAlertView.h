...
typedef enum {
    ...
    BBAlertViewStyle1,              //图书里面用的        未使用
    BBAlertViewStyleCustomView,     //可放入自定义的view
    BBAlertViewStyleLottery,        //彩票里面使用        未使用
    BBAlertViewStyleMessageFilter,  //精准营销            未使用
    BBAlertViewStyleOrderRedpacket,  //订单红包
}BBAlertViewStyle;
...
@property (nonatomic, strong) UIView *seplineH;  //横向分割线,按钮上沿
@property (nonatomic, strong) UIView *seplineV;  //竖直分割线，按钮之间
...
/*!
 @abstract      初始话方法，默认的style：BBAlertViewStyleDefault
 @param         title  标题
 @param         atrributeMessage  富文本
 @param         delegate  代理
 @param         atrributeDelegate 点击富文本的代理
 @param         cancelButtonTitle  取消按钮title
 @param         otherButtonTitle  其他按钮，如确定
 @result        BBAlertView的对象
 */
 ...
 *  @param image            图片 可以无
 *  @param delegate         代理
 *  @param cancelImage     取消按钮图片
 *  @param otherButtonTitle 确定按钮
 *
 *  @return BBAlertView的对象
 */
-(id)initWithTitle:(NSString *)title
           message:(NSString *)message
               image:(UIImage *)image
